    ...
    case Transaction::OP_SETALLOCHINT:
      {
        ...
        if (_check_replay_guard(cid, oid, spos) > 0)
          r = _set_alloc_hint(cid, oid, expected_size, expected_write_size,
...
{
  dout(15) << "set_alloc_hint " << cid << "/" << oid << " size " << expected_size << " write_size " << expected_write_size << " size_prob " << (int)expected_size_probability << dendl;
  ...
  hint = MIN(expected_write_size, m_filestore_max_alloc_hint_size);

  ret = backend->set_alloc_hint(**fd, hint);

  lfn_close(fd);
out:
  dout(10) << "set_alloc_hint " << cid << "/" << oid << " size " << expected_size << " write_size " << expected_write_size << " size_prob " << (int)expected_size_probability << " hint " << hint << " = " << ret << dendl;
  assert(!m_filestore_fail_eio || ret != -EIO);
  return ret;
}
